"""
database.py — V2.0 SQLite schema, connection management, and Repository.

V2.0 changes
-------------
* ``tc_no`` → ``phone`` (E.164 string) everywhere.
* ``animals`` table gains ``total_price_kurus`` and ``total_weight_grams``
  (stored as INTEGER for exact precision — no floats).
* ``Decimal`` ↔ INTEGER conversion happens at the repository boundary.
* Added **paginated search** to prevent GUI freezing on large datasets.
* All operations use Python ``logging`` instead of silent failures.
* Paths resolved via ``__file__`` for Nuitka compatibility.
* ``ON DELETE CASCADE`` on both FKs in ``animal_shares``.
"""
...
def _get_app_dir() -> Path:
    ...
    whether running as script or compiled Nuitka binary."""
    if getattr(sys, "frozen", False):
        # Nuitka / PyInstaller compiled
...
def _get_connection(
    ...
    """Yield a connection with FK enforcement, WAL journal, and Row factory."""
...
class KurbanRepository:
    ...
    def commit_staged_animals(self, staged: List[StagedAnimal]) -> List[int]:
        ...
        Returns newly-created ``animal.id`` values.
        Raises ``sqlite3.Error`` on any failure (full rollback).
        """
            ...
            try:
                ...
                for animal in staged:
                    ...
                    for sh in animal.shareholders:
                        # UPSERT on phone PK — latest name wins.
                        cursor.execute(
                        ...
                        cursor.execute(
                            ...
                            "VALUES (?, ?, ?)",
                            (animal_id, sh.phone, int(sh.is_paid)),
                        )
                conn.commit()
                logger.info(
    ...
    def update_payment_status(
        ...
        """Toggle one shareholder's payment flag.

        Raises ``sqlite3.Error`` on lock / unavailability.
        """
        with _get_connection(self._db_path) as conn:
            conn.execute(
    ...
    def update_animal(
        ...
        """Update an animal's core fields."""
        with _get_connection(self._db_path) as conn:
            conn.execute(
    ...
    def search_by_animal_id(self, animal_id: int) -> Optional[AnimalRecord]:
        """Return a single animal with all its shares, or ``None``."""
        with _get_connection(self._db_path) as conn:
            rows = conn.execute(
    ...
    def search_by_phone_or_name(self, query: str) -> List[AnimalRecord]:
        """Find animals where *any* shareholder matches by phone or name."""
        like_query = f"%{query}%"
        with _get_connection(self._db_path) as conn:
            animal_ids = [
                row["animal_id"]
    ...
    def get_animals_paginated(
        ...
        """Return one page of fully-hydrated animals (newest first)."""
    ...
    def get_all_for_export(self) -> List[AnimalRecord]:
        """Return every animal — used only for Excel export."""
        with _get_connection(self._db_path) as conn:
            rows = conn.execute(
